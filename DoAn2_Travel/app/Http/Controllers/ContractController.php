...
class ContractController extends Controller
{
    public function new_contract(){
        if(Session::get('position_id')==3 || Session::get('position_id')==1){
    ...
    public function save_contract(Request $request){
        ...
        $contract->contract_id=$request['booking_id'];
    ...
    public function list_contract( $contract_status){
        if(Session::get('position_id')==3 || Session::get('position_id')==1){
            $contract= Contract::where('contract_status',$contract_status)->join('tbl_booking','tbl_contract.contract_id','=','tbl_booking.booking_id')
    ...
    public function detail_contract($contract_id){
        if(Session::get('position_id')==3 || Session::get('position_id')==1){
            $contract=Contract::Where('contract_id',$contract_id)->join('tbl_staff','tbl_staff.staff_id','=','tbl_contract.staff_id')->first();
            $booking=Booking::where('booking_id',$contract->contract_id)->join('tbl_customer','tbl_booking.customer_id','=','tbl_customer.customer_id')->first();
            return view('Admin_Views.Booking_Management.Detail_contract')->with(compact('contract'))->with(compact('booking'));
        }else{
            Session::put('message','*you not have this permission');
            return redirect()->back();
        }
    ...
    public function update(Request $request){
        ...
        $contract = Contract::where('contract_id',$request['contract_id'])->first();
